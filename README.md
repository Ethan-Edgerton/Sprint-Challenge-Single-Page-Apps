...
> Answer:
    Client-side routing is when an entire website is downloaded at once, which allows super fast transitions from page to page, since it is already prepared. Links are blocked and replaced with state changes, which then load new components/elements. 

    Server-side routing downloads only a page at a time, which leads to shorter initial loads, but longer transitions between pages and uses more data in the long term by requesting the whole page each time, rather than only what is needed.
